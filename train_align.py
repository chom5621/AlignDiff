...
from models.molopt_score_model_align import ScorePosNet3D
...
if __name__ == '__main__':
    root_dir = '/home/chom5621/AlignDiff'
    ...
    wandb.init(project="sbdd", config=args)
    ...
    wandb.log({'Trainable Params':misc.count_parameters(model)})
    ...
    def train(it):
        ...
        if it % args.train_report_iter == 0:
            metrics = {
                'Iteration': it,
                'Train Loss': loss.item(),  # Ensure to call .item() to log as scalar
                'Train Pos Loss': loss_pos.item(),
                'Train v Loss': loss_v.item(),
                'Learning Rate': optimizer.param_groups[0]['lr'],
                'Gradient Norm': orig_grad_norm.item(),
                # 'V Loss': loss_v.item(),  # Uncomment if you want to log this
            }
            wandb.log(metrics)
            logger.info(
    ...
    def validate(it):
        ...
        wandb.log({'Val Loss': avg_loss, 'Val Pos Loss':avg_loss_pos, 'Val v Loss':avg_loss_v, #'Val emb Loss':avg_loss_emb, 
    ...
    try:
        ...
        for it in range(1, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                if it == 235000:
                    # Train loss가 converge하는 iteration이 235000일 때, checkpoint를 저장
