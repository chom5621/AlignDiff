...
def continuous_var_bayesian_update(alphas_cumprod, alphas_cumprod_prev, x):
        """
        x: [N, D]
        """
        """
        ...
        # beta_t = sigma1^(-2t)
        # betas[t] = sigma1^(-2t)
        gamma = alphas_cumprod/alphas_cumprod_prev #1 - torch.pow(sigma1, 2 * t)  # [B]
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, bins=13):
        super().__init__()
        self.config = config
        ...
        if self.time_emb_dim > 0:
            if self.time_emb_mode == 'simple':
                self.ligand_atom_emb = nn.Linear(ligand_atom_feature_dim + 1, emb_dim) # 13+1, 127
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand, 
                time_step=None, return_all=False, fix_x=False, hbap_protein_prev=None, hbap_ligand_prev=None, hbap_protein=None, hbap_ligand=None):

        batch_size = batch_protein.max().item() + 1
        init_ligand_v = F.one_hot(init_ligand_v.to(torch.int64), self.num_classes).float()
        if self.time_emb_dim > 0:
            if self.time_emb_mode == 'simple':
    ...
    def get_diffusion_loss(
        ...
        ligand_pos_perturbed = a_pos.sqrt() * ligand_pos + (1.0 - a_pos).sqrt() * pos_noise + k_t_pos * shift_cond_t # pos_noise * std
        ...
        pred_ligand_pos, pred_ligand_v, pred_emb = preds['pred_ligand_pos'], preds['pred_ligand_v'], preds['final_ligand_h']
        ...
        # 연속형 변수인 position을 학습하는 loss_pos
        loss_pos = scatter_mean(((pred - target) ** 2).sum(-1), batch_ligand, dim=0)
        unique_values = torch.ones(len(torch.unique(batch_ligand)))*2
        for idx in range(len(torch.unique(batch_ligand))): 
            mask = (batch_ligand == idx)
            if mask.any():
                unique_values[idx] = a_pos[mask][0]
        loss_pos = loss_pos * unique_values.to(loss_pos.device)/2       
        loss_pos = torch.mean(loss_pos)
        
        # 이산형 변수인 atom type과 aromatic을 학습하되, cross_entroy_loss를 쓰지 않고 loss_pos과 align되도록 학습하는 loss_v 제안
        pred_v = F.softmax(pred_ligand_v, dim=-1)
        true_v = F.one_hot(ligand_v, self.num_classes)
        loss_v = scatter_mean(((pred_v - true_v) ** 2).sum(-1), batch_ligand, dim=0)
        loss_v = loss_v * unique_values.to(loss_v.device)/2 * self.num_classes
        loss_v = torch.mean(loss_v)
        loss = loss_pos + loss_v # loss_v에 따로 weight 곱해줄 필요 없음       
